...
import { RetryConfig, retryer } from '@vendasta/core/vax';
 ...
 * accountX.retry();
 */
export class APIStream<StreamResult, APIResponse = StreamResult> implements XStream<StreamResult> {
    ...
    public static of<StreamResult, APIResponse = StreamResult>(
        ...
        return new APIStream(streamInitializer, false);
    }

    public static withRetry<StreamResult, APIResponse = StreamResult>(
        streamInitializer: APIStreamInitializer<StreamResult, APIResponse>,
        retryConfig?: RetryConfig,
    ): APIStream<StreamResult, APIResponse> {
        return new APIStream(streamInitializer, true, retryConfig);
    }
    ...
    constructor(
        ...
        this.results$ = this.retries$$.pipe(
            startWith(null),
            switchMap(() => streamInitializer(this.handleAPICall.bind(this), this.handleError.bind(this))),
            shareReplay(1),
        );
        ...
        this.loading$ = this.outStandingCalls$$.pipe(
            ...
            merge(this.results$.pipe(switchMap(() => new EmptyObservable<boolean>()))),
            distinctUntilChanged(),
        );
    }

    private handleAPICall(call: Observable<APIResponse>): Observable<APIResponse> {
        this.outStandingCalls$$.next(1);
        if (this.shouldRetry) {
            call = call.pipe(retryer(this.retryConfig));
        }
        return call.pipe(
            catchError(err => {
